...
def create_server() -> FastMCP:
    ...
    # ── Mac MCP backends ────────────────────────────────────────────────────
    # Registered as direct tools (same pattern as NAS host-tools) so we can
    # send Bearer auth and verify TLS against the internal CA.
    #
    # MAC_MCP_BACKENDS=mac-studio=https://10.10.0.3:3456,mac-notebook=https://10.10.0.4:3456
    # MAC_MCP_API_KEY=<shared-secret>

    _mac_backends_raw = os.environ.get("MAC_MCP_BACKENDS", "")
    _mac_api_key = os.environ.get("MAC_MCP_API_KEY", "")

    if _mac_backends_raw:
        import asyncio
        import json

        mac_backends: list[tuple[str, str]] = []
        for entry in _mac_backends_raw.split(","):
            entry = entry.strip()
            if not entry or "=" not in entry:
                continue
            bname, burl = entry.split("=", 1)
            mac_backends.append((bname.strip(), burl.strip().rstrip("/")))

        async def _mac_http(
            base_url: str, method: str, path: str, body: dict | None = None,
        ) -> dict:
            """HTTP call to a mac-mcp instance with TLS + Bearer auth."""
            ckw: dict = {"timeout": 60}
            if _SSL_CA_CERTFILE:
                ckw["verify"] = _SSL_CA_CERTFILE
            headers: dict[str, str] = {}
            if _mac_api_key:
                headers["Authorization"] = f"Bearer {_mac_api_key}"
            async with httpx.AsyncClient(**ckw) as client:
        ...
        async def _pick_mac_backend() -> tuple[str, str] | None:
            """Return (name, url) of the highest-priority reachable backend."""
            best: tuple[str, str] | None = None
            best_pri = -1
            for bname, burl in mac_backends:
                try:
                    data = await _mac_http(burl, "GET", "/health")
        ...
        async def _register_mac_tools() -> None:
            # Discover full tool definitions from first reachable backend
            tools: list[dict] = []
            for bname, burl in mac_backends:
                try:
                    tools = await _mac_http(burl, "GET", "/tools")
                    log.info("Discovered %d mac-mcp tools from %s", len(tools), bname)
                    break
                except Exception as e:
                    log.warning("Cannot reach %s for discovery: %s", bname, e)

            if not tools:
                log.warning("No mac-mcp backends reachable — no Apple tools registered")
                return
            ...
            for tool_def in tools:
                ...
                def _make_handler(tool_name: str):
                    async def handler(**kwargs) -> str:
                        backend = await _pick_mac_backend()
                        if not backend:
                            return "ERROR: No mac-mcp backend reachable"
                        bname, burl = backend
                        try:
                            result = await _mac_http(
                                burl, "POST", "/tool",
                                {"name": tool_name, "arguments": kwargs},
                            )
                            return json.dumps(result, indent=2)
                        except Exception as e:
                            return f"Error calling {tool_name} on {bname}: {e}"
                    return handler

                gw.tool(name=tname, description=tdesc)(_make_handler(tname))

        asyncio.get_event_loop().run_until_complete(_register_mac_tools())
